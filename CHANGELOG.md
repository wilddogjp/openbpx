...
- Agent plugins now include bundled `bpx` binaries for Linux, macOS, and Windows (`amd64`/`arm64`) plus a wrapper fallback for plugin-only use.
...
- README install guidance now leads with Agent Plugins, documents the bundled CLI fallback, and keeps direct SKILL.md generation under Legacy Agent Skills.
